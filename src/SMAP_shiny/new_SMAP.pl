...
=head1 SYNOPSIS
	...
	where each option is followed by a value as follows:
	  ...
	  --output,-o			Specify an output filename (required)
	  --plex			Multiplex number of the isobaric labeling approach (default: 11)
	  -fc				Signal to Noise ratio (optional; default: 3)
	  --log2			Indicating whether signals in variant peptide table are log2 transformed (default: no)
	  -nl				The upper threshold of a noise level (default: data derived)
	...
	while(<MUTATION>)
	{
		...
		if($log2)
		{
			for(my $i=4;$i<=$#data;$i++)
			{
				if($data[$i]>100)
				{
					print "WARNING: MAKE SURE YOU INPUT LOG2 TRANSFORMED DATA:\n";
				}
				$data[$i] = 2**$data[$i];
			}
		}
...
sub genotype_matching
{
	...
	print "Sample ID\tInferred ID\tCscore\tDeltaCscore\n";
	foreach my $MS_sample (sort {$a<=>$b} keys %match_ratio)
	{
		...
		foreach my $genotype_sample (reverse sort {$match_ratio{$MS_sample}{$a}<=>$match_ratio{$MS_sample}{$b}} keys %{$match_ratio{$MS_sample}})
		{
			...
			if($loop == 1)
			{
				...
				print $$header_data[$MS_sample-$plex],"\t",$genotype_sample,"\t",$match_ratio{$MS_sample}{$genotype_sample},"\t";
			}
			if($loop == 2)
			{
	...
	print CSCORE "Sample ID\tInferred ID\tCscore\tDeltaCscore\n";
	foreach my $MS_sample (sort {$a<=>$b} keys %match_ratio)
	{
		my $loop = 0;
		my $top_ratio = 0;
